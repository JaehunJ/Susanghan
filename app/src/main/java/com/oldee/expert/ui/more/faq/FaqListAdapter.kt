...
import com.oldee.expert.databinding.LayoutSimpleListItemBinding
import com.oldee.expert.retrofit.response.FaqData

class FaqListAdapter(val onClick: (FaqData) -> Unit) :
    ListAdapter<FaqData, FaqListAdapter.FaqItemViewHolder>(
        FaqDiffUtil()
    ) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =
        FaqItemViewHolder.from(parent)

    override fun onBindViewHolder(holder: FaqItemViewHolder, position: Int) {
        holder.bind(getItem(position), onClick)
    }

    class FaqItemViewHolder(val binding: LayoutSimpleListItemBinding) :
